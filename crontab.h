...
// reference from https://github.com/mariusbancila/croncpp v2023.03.30
...
inline std::tm to_tm(CRONCPP_STRING_VIEW time) {
  ...
  int minute = 0;
  int second = 0;
  sscanf_s(time.data(), "%d-%d-%d %d:%d:%d", &year, &month, &day, &hour,
           &minute, &second);
...
static std::vector<std::string> split(CRONCPP_STRING_VIEW text,
...
CRONCPP_CONSTEXPTR inline bool contains(CRONCPP_STRING_VIEW text,
...
static std::pair<cron_int, cron_int> make_range(CRONCPP_STRING_VIEW field,
...
static bool find_next(cronexpr const& cex, std::tm& date, size_t const dot) {
  ...
  auto updated_second = find_next(
      cex.seconds, date, Traits::CRON_MIN_SECONDS, Traits::CRON_MAX_SECONDS,
      second, cron_field::second, cron_field::minute, empty_list);
  ...
  auto update_minute = find_next(
      cex.minutes, date, Traits::CRON_MIN_MINUTES, Traits::CRON_MAX_MINUTES,
  ...
  auto updated_hour = find_next(
      cex.hours, date, Traits::CRON_MIN_HOURS, Traits::CRON_MAX_HOURS, hour,
  ...
  auto updated_month = find_next(
      cex.months, date, Traits::CRON_MIN_MONTHS, Traits::CRON_MAX_MONTHS, month,
